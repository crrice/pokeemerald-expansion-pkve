...
static void DrawMonIcon(u16 species);
...
static bool8 StatEditor_DoGfxSetup(void)
{
    ...
    case 4:
        ...
        DrawMonIcon(sStatEditorDataPtr->speciesID);
        gMain.state++;
        break;
    case 5:
...
static bool8 StatEditor_LoadGraphics(void)
{
    ...
    case 1:
        ...
        {
            LZ77UnCompWram(sStatEditorBgTilemap, sBg1TilemapBuffer);
            sStatEditorDataPtr->gfxLoadState++;
        }
        break;
    case 2:
...
static void DrawMonIcon(u16 species)
{
    sStatEditorDataPtr->monIconSpriteId = CreateMonPicSprite_Affine(species, 0, 0x8000, TRUE, MON_ICON_X, MON_ICON_Y, 0, TAG_NONE);
...
static void PrintMonStats(void)
{
    ...
    // Pokemon info: name, level, gender
...
static void SelectorCallback(struct Sprite *sprite)
{
    static const struct SpriteCordsStruct spriteCords[6][2] = {
        {{188, 50}, {220, 50}},
        {{188, 66}, {220, 66}},
        {{188, 82}, {220, 82}},
        {{188, 98}, {220, 98}},
...
static void Task_StatEditorMain(u8 taskId)
{
    ...
    if (JOY_NEW(DPAD_LEFT) || JOY_NEW(DPAD_RIGHT))
    {
        ...
        {
            if (!FlagGet(FLAG_STAT_EDITOR_IV_UNLOCK))
            {
                PlaySE(SE_FAILURE);
                return;
            }
...
static void ChangeAndUpdateStat(void)
{
